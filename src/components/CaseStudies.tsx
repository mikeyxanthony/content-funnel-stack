    ...
    <section id="cases" className="py-20 px-4">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-12">
          <span className="text-[#39FF14] text-xs font-semibold uppercase tracking-widest">Track Record</span>
          <h2 className="text-3xl sm:text-4xl font-black mt-3 mb-4">Case Studies and <span className="text-[#39FF14] neon-glow">Proof</span></h2>
        </div>
        <div className="grid md:grid-cols-3 gap-6">
          {cases.map((c) => (
            <div key={c.name} className="neon-border rounded-2xl bg-[#111111] p-6 flex flex-col gap-4">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-[#39FF14]/20 border border-[#39FF14]/40 flex items-center justify-center text-[#39FF14] font-bold text-sm flex-shrink-0">{c.initials}</div>
                <div><p className="text-white font-bold text-sm">{c.name}</p><p className="text-[#39FF14] text-xs font-semibold">{c.result}</p></div>
              </div>
              <div className="space-y-3 flex-1">
                ...
                <div className="bg-[#39FF14]/5 border border-[#39FF14]/20 rounded-lg p-3"><p className="text-[#39FF14] text-xs font-semibold uppercase tracking-wide mb-1">After</p><p className="text-white/70 text-xs leading-relaxed">{c.after}</p></div>
              </div>
            </div>
          ))}
        </div>
