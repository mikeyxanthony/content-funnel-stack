    ...
    <section id="who" className="py-20 px-4">
      <div className="max-w-5xl mx-auto">
        <div className="text-center mb-12">
          <span className="text-[#39FF14] text-xs font-semibold uppercase tracking-widest">Is This For You?</span>
          <h2 className="text-3xl sm:text-4xl font-black mt-3">This Is <span className="text-[#39FF14] neon-glow">Specifically Built</span> For You If</h2>
        </div>
        <div className="grid md:grid-cols-2 gap-6">
          <div className="neon-border rounded-2xl bg-[#111111] p-8">
            <div className="flex items-center gap-2 mb-6"><span className="text-[#39FF14] text-xl">✅</span><h3 className="text-white font-bold text-lg">This IS for you if</h3></div>
            <ul className="space-y-3">{forList.map((item,i) => (<li key={i} className="flex items-start gap-3 text-white/75 text-sm"><span className="text-[#39FF14] mt-0.5 flex-shrink-0">◆</span>{item}</li>))}</ul>
          </div>
